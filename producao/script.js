...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (stepFormOverlay) {
        ...
        const submitForm = () => {
            ...
            // 3. Google Sheets Webhook (via hidden iframe form — bypasses CORS)
            if (typeof SAILORS_CONFIG !== 'undefined' && SAILORS_CONFIG.webhookUrl) {
                try {
                    // Create hidden iframe target
                    let iframe = document.getElementById('_sailors_hidden_iframe');
                    if (!iframe) {
                        iframe = document.createElement('iframe');
                        iframe.id = '_sailors_hidden_iframe';
                        iframe.name = '_sailors_hidden_iframe';
                        iframe.style.display = 'none';
                        document.body.appendChild(iframe);
                    }

                    // Create and submit a real HTML form (immune to CORS)
                    const hiddenForm = document.createElement('form');
                    hiddenForm.method = 'POST';
                    hiddenForm.action = SAILORS_CONFIG.webhookUrl;
                    hiddenForm.target = '_sailors_hidden_iframe';
                    ...
                    console.log('Lead enviado via form submission para Google Sheets.');
                } catch (err) {
                    console.error('Erro ao enviar lead via form:', err);
                    ...
                    }).catch(err2 => console.error('Fallback fetch também falhou:', err2));
                }
            }
